  ...
  return new Promise<boolean>(async (resolve, reject) => {
    ...
    if (status.canAskAgain) {
      ...
      if(rationaleAccepted) {
        ...
        if(!granted) showNotGrantedErrorMessage(request.notGrantedErrorMessage)
        resolve(granted)
      } else {
      ...
      if(!newStatus.granted) showNotGrantedErrorMessage(request.notGrantedErrorMessage)
      resolve(newStatus.granted)
    }
  })
}

function getPermissionStatus(type: PermissionType): Promise<ExpoPermissions.PermissionResponse> {
...
function askPermission(type: PermissionType): Promise<boolean> {
  const expoPermissionType = getExpoPermissionType(type);
  return ExpoPermissions.askAsync(expoPermissionType).then(result => result.granted)
}
...
function getExpoPermissionType(type: PermissionType): ExpoPermissions.PermissionType {
  switch (type) {
